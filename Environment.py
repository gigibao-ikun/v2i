...
import random
import math
# This file is revised for more precise and concise expression.# 此文件经过修改，以便表达更加精确和简洁。
class V2Vchannels:              
...
class Environ:
    ...
    def test_channel(self):
        # ===================================
        #   test the V2I and the V2V channel 测试 V2I 和 V2V 通道
        # ===================================
        self.n_step = 0
        self.vehicles = []
        n_Veh = 60
    ...
    def renew_neighbor(self):   
        # ==========================================
        # update the neighbors of each vehicle.更新每辆车的邻居。
        # ===========================================
    ...
    def renew_channel(self):
        # ===========================================================================
        # This function updates all the channels including V2V and V2I channels此功能更新所有通道，包括 V2V 和 V2I 通道
        # =============================================================================
        positions = [c.position for c in self.vehicles]
        self.V2Ichannels.update_positions(positions)
        self.V2Vchannels.update_positions(positions)
    ...
    def Compute_Performance_Reward_fast_fading_with_power(self, actions_power):   # revising based on the fast fading part
        ...
        # remove the effects of none active links# 删除非活动链接的影响
        ...
        V2I_Rate = np.log2(1 + np.divide(10**(V2I_Signals/10), self.V2I_Interference[0:min(self.n_RB,self.n_Veh)]))
         ...
         # -- compute the latency constraits --#——计算延迟约束——
